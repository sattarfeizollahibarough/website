...
menu:
  main:
    ...
    weight: 10
    post: >
      <p>آماده هستید ک دست به کار شویم? میخواهیم یک کلاستر ساده کوبرنتیز بسازیم که یک اپلیکیشن را اجرا می‌کند.</p>
card:
  name: tutorials
  weight: 10
---
...
کاتاکودا برای شما یک محیط کوبرنتیز مبتنی بر مرورگر وب فراهم میاورد.

{{< note >}}
شما همچنین می‌توانید از این مستند نیز بهره بگیرد از [Minikube](/docs/tasks/tools/install-minikube/) را بر روی سیستم خود نصب نموده‌اید.
{{< /note >}}
...
* اجرای اپلیکیشن.
* بررسی لاگهای تولید شده توسط اپلیکیشن.
...
در این آموزش از یک تصویر کانتینری که از NGINX بهره برده است که به درخواستها پاسخ می‌دهد استفاده شده است.
...
3. فقط در سیستم کاتاکودا: در بالای ترمینال بر روی علامت بعلاوه کلیک کرده و سپس بر روی گزینه **Select port to view on Host 1**. کلیک کنید

4. فقط در سیستم کاتاکودا: مقدار `30000` را تایپ کرده و بر روی, **Display Port**. کلیک کنید

## ساخت استقرار

در کوبرنتیز [*Pod*](/docs/concepts/workloads/pods/) به مجموعه‌ای از یک تا چند کانتینر اشاره دارد که با هدف مدیریت واحد آنها بوجود آمده‌اند.
در این آموز Pod ما فقط شامل یک کانتینر است.
در کوبرنتیز
